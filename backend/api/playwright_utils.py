...
# ─── Playwright Frame Extraction ────────────────────────────────────────────

# ─── Playwright Helper ──────────────────────────────────────────────────────

async def _ensure_target_video(page, is_permalink: bool) -> None:
    """
    Ensures that the target video is correctly located, tagged with ID 'haqq-target-video',
    styled fullscreen to overlay all other page elements, and is playing/muted.
    """
    if not is_permalink:
        # CDN direct URL: make sure it's playing and muted
        await page.evaluate("""
            () => {
    ...
    # Permalink URL (TikTok, FB, etc.)
    await page.evaluate("""
        () => {
            // Check if the current tagged video still exists and is attached to document
            let v = document.getElementById('haqq-target-video');
            if (v && document.body.contains(v)) {
                // Ensure muted and playing
                v.muted = true;
                v.play().catch(() => {});
                
                // Ensure fullscreen styles are intact (in case framework reset them)
            ...
            if (!bestVid) {
                bestVid = document.querySelector('video');
            }
            ...
            if (bestVid) {
                ...
                if (bestVid.parentElement !== document.body) {
                    document.body.appendChild(bestVid);
                }
                bestVid.style.position = 'fixed';
                bestVid.style.top = '0';
                bestVid.style.left = '0';
                bestVid.style.width = '100vw';
                bestVid.style.height = '100vh';
                bestVid.style.zIndex = '2147483647';
                bestVid.style.background = 'black';
                bestVid.style.objectFit = 'contain';
                bestVid.style.visibility = 'visible';
                bestVid.style.opacity = '1';

                // Hide overlays
                document.querySelectorAll('*').forEach(el => {
                    if (el === bestVid || bestVid.contains(el)) return;
    ...
    async with async_playwright() as p:
        ...
        try:
            if is_permalink:
                ...
                # 2. Find and tag the main target video
                await _ensure_target_video(page, is_permalink)
                ...
                # Ensure direct custom video is playing
                await _ensure_target_video(page, is_permalink)
            ...
            has_duration = await page.evaluate(
                """() => new Promise((resolve) => {
                    const v = document.getElementById('haqq-target-video') || document.querySelector('video');
            ...
            if has_duration:
                ...
                await page.evaluate(
                    "() => { const v = document.getElementById('haqq-target-video') || document.querySelector('video'); if (v) v.currentTime = 0; }"
                )
            ...
            if has_duration:
                duration = await page.evaluate(
                    "() => { const v = document.getElementById('haqq-target-video') || document.querySelector('video'); return v ? v.duration : 0; }"
                )
                ...
                await page.evaluate(
                    "() => { const v = document.getElementById('haqq-target-video') || document.querySelector('video'); if (v) v.pause(); }"
                )
                ...
                for t in timestamps:
                    ...
                    actual_t = await page.evaluate(
                        "() => { const v = document.getElementById('haqq-target-video') || document.querySelector('video'); return v ? v.currentTime : 0; }"
                    )

                    # Ensure target video is correctly structured & not unmounted/re-rendered by page framework
                    await _ensure_target_video(page, is_permalink)
...
# ─── Thread Isolation (Windows Proactor Loop) ───────────────────────────────
